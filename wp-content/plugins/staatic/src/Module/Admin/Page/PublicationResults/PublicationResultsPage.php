...
final class PublicationResultsPage implements ModuleInterface
{
    ...
    public function hooks()
    {
        ...
        $this->listTable->registerHooks('staatic_page_staatic-publication-results');
    }
    ...
    public function render()
    {
        ...
        $this->listTable->setArguments([
            'baseUrl' => admin_url(\sprintf('admin.php?page=staatic-publication-results&id=%s', $publication->id())),
            'buildId' => $publication->build()->id()
        ]);
        $listTable = $this->listTable->wpListTable();
        $listTable->prepare_items();
        $this->renderer->render('admin/publication/results.php', \compact('publication', 'listTable'));
    }
}
