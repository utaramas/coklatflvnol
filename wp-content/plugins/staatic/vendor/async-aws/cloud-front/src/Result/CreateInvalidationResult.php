...
class CreateInvalidationResult extends Result
{
    ...
    protected function populateResult($response)
    {
        ...
        $this->invalidation = new Invalidation(['Id' => (string) $data->Id, 'Status' => (string) $data->Status, 'CreateTime' => new \DateTimeImmutable((string) $data->CreateTime), 'InvalidationBatch' => new InvalidationBatch(['Paths' => new Paths(['Quantity' => (int) (string) $data->InvalidationBatch->Paths->Quantity, 'Items' => !$data->InvalidationBatch->Paths->Items ? [] : $this->populateResultPathList($data->InvalidationBatch->Paths->Items)]), 'CallerReference' => (string) $data->InvalidationBatch->CallerReference])]);
    }
