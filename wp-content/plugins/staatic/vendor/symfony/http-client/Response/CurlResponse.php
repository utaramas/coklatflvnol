...
final class CurlResponse implements ResponseInterface, StreamableInterface
{
    ...
    private static function perform(ClientState $multi, array &$responses = null)
    {
        ...
        try {
            ...
            while ($info = \curl_multi_info_read($multi->handle)) {
                ...
                $multi->handlesActivity[$id][] = null;
                $multi->handlesActivity[$id][] = \in_array($result, [\CURLE_OK, \CURLE_TOO_MANY_REDIRECTS], \true) || '_0' === $waitFor || \curl_getinfo($ch, \CURLINFO_SIZE_DOWNLOAD) === \curl_getinfo($ch, \CURLINFO_CONTENT_LENGTH_DOWNLOAD) ? null : new TransportException(\sprintf('%s for "%s".', \curl_strerror($result), \curl_getinfo($ch, \CURLINFO_EFFECTIVE_URL)));
            }
        } finally {
     ...
     */
    private static function parseHeaderLine($ch, string $data, array &$info, array &$headers, $options, CurlClientState $multi, int $id, &$location, $resolveRedirect, $logger, &$content = null) : int
    {
        $waitFor = @\curl_getinfo($ch, \CURLINFO_PRIVATE) ?: '_0';
        if ('H' !== $waitFor[0]) {
            return \strlen($data);
