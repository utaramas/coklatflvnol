...
class MockResponse implements ResponseInterface, StreamableInterface
{
    ...
    {
        $this->body = is_array($body) || $body instanceof \Traversable ? $body : (string) $body;
    ...
    private static function readResponse(self $response, array $options, ResponseInterface $mock, int &$offset)
    {
        ...
        $response->info = ['start_time' => $response->info['start_time'], 'user_data' => $response->info['user_data'], 'http_code' => $response->info['http_code']] + $info + $response->info;
        if (!isset($response->info['total_time'])) {
            $response->info['total_time'] = \microtime(\true) - $response->info['start_time'];
        }
